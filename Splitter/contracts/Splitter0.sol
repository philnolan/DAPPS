...
contract Splitter0 is Owned {
	...
	function Splitter0(address bobAddr, address carolAddr) {
		bob = Beneficiary({ addr: bobAddr, available:0, totalClaimed:0 });
		carol = Beneficiary({ addr: carolAddr, available:0, totalClaimed:0 });
	}
	...
	function getBeneficiaryDetail(address beneficiaryAddr) constant returns(uint available, uint totalClaimed) {
		if (beneficiaryAddr == bob.addr) {
			return  (bob.available,carol.totalClaimed);
		}
		else {
